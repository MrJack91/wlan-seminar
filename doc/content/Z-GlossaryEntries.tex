...
\newglossaryentry{glos:cloudLabel}{
	name=Cloud,
	description={\textit{Cloud} bietet dem Benutzer Software oder Hardware zur Verfügung an, ohne dass jener im Detail weiss, auf welcher Infrastruktur er arbeitet. Meist stehen Komponenten im Internet bereit, so dass ein weltweiter Zugriff ermöglicht wird.}
}
...
\newglossaryentry{glos:saltLabel}{
	name={Salt},
	description={Ein \textit{Salt} beinhaltet zufällige Daten und wird zum Hash mitgerechnet. So können bereits vorhandene \glspl{glos:rainbowTableLabel} nicht verwendet werden, da die höchstwahrscheinlich nicht mit dem selben \textit{Salt} berechnet wurden.}
}
...
\newglossaryentry{ssidLabel}{
	name={SSID},
	description={Mit der \textit{Service Set Identifier (SSID)} wird ein gesamtes Netzwerk identifiziert. Die \textit{SSID} entspricht dem sichtbaren Netzwerknamen und kann sich über mehrere \gls{apLabel}'s erstrecken.\footcite{Understanding_the_Network_Terms_SSID_BSSID_and_ESSID_Technical_Documentation_Support_2015-04-10}},
...
\newglossaryentry{bssidLabel}{
	name={BSSID},
	description={Mit der \textit{Basic Service Set Identifier (BSSID)} wird ein \gls{apLabel} identifiziert.
	...
	first={Basic Service Set Identifier (BSSID)},
